...
const DECK_FORMAT_ORDER = ['commander', 'standard', 'modern', 'pioneer', 'legacy', 'vintage', 'pauper', 'brawl', 'oathbreaker', 'other'];
const DECK_BOARDS = { commander: 'Commander', main: 'Mainboard', side: 'Sideboard', maybe: 'Maybeboard' };
...
function deckById(id) { return (collection.decks || []).find(d => d.id === id); }
function deckFormat(deck) { return DECK_FORMATS[deck.format] || DECK_FORMATS.other; }

function sidImageUrl(sid, size = 'normal') {
...
function deckCardOwnedQty(dc, maps) {
...
function deckStats(deck, maps = deckOwnedMaps()) {
...
// ── Deck CRUD ────────────────────────────────────────────────────────────────
function createDeck(name, format = 'commander') {
...
function addCardToDeck(deck, src, board = 'main', qty = 1) {
...
function addOwnedCardToDeck(card, deck) {
  addCardToDeck(deck, {
...
function ctxDeckSubmenu(applyFn) {
...
function captureScryfallCard(card) {
...
function renderDeckTile(deck, maps) {
  const f = deckFormat(deck);
...
function renderDeckBoard(deck, board, cards, maps) {
  const f = deckFormat(deck);
...
function renderDeckCardRow(dc, maps) {
  const qty = dc.quantity || 1;
...
let _deckAddResults = [];   // rows currently shown in the Add Cards modal

function showDeckAddCardModal(deckId) {
...
// ── Deck context menus ───────────────────────────────────────────────────────
function showDeckTileContextMenu(x, y, deck) {
  showContextMenu(x, y, [
    { header: deck.name },
...
function showDeckCardContextMenu(x, y, deck, dc) {
  const f = deckFormat(deck);
...
function showDeckExportModal(deckId) {
  const deck = deckById(deckId);
  if (!deck) return;
...
function _ctxDismiss(e) { if (_ctxEl && !_ctxEl.contains(e.target)) closeContextMenu(); }
function _ctxKey(e)     { if (e.key === 'Escape') closeContextMenu(); }
function _ctxWheel(e)   { if (_ctxEl && !_ctxEl.contains(e.target)) closeContextMenu(); }
...
function promptText(title, placeholder, cb) {
  showModal(`
...
function ctxBinderSubmenu(applyFn, currentBinder = null) {
...
// ── Card actions ─────────────────────────────────────────────────────────────
function moveCardToBinder(card, binder) {
...
async function deleteCardEntry(card) {
...
function openCardOnScryfall(card) {
...
function copyToClipboard(text, what = 'Copied') {
...
function addSlCardToCollection(scryfallId, binder, opts = {}) {
...
function showSlCardContextMenu(x, y, scryfallId) {
...
function addDropCardsToBinder(drop, ids, binder) {
...
function showSlDropContextMenu(x, y, drop) {
...
function showSuperdropContextMenu(x, y, superdrop) {
...
function showSealedContextMenu(x, y, item) {
