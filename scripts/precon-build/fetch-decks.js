...
// types, and fetches each deck file into cache/decks/ (gitignored, resumable —
...
const UA = 'SecretLairTracker/0.28.0 (https://github.com/ArielKaplan/magic_collection_tracker; akaplan.nj@gmail.com)';
...
// half-decks — later, behind a toggle), MTGO/Arena/Shandalar/DotP (digital),
...
(async () => {
  process.stdout.write('MTGJSON DeckList.json … ');
  ...
  const worker = async () => {
    while (queue.length) {
      ...
      try {
        ...
        fetched++;
        if ((fetched + failed) % 50 === 0) console.log(`  … ${fetched} fetched, ${cached} cached, ${failed} failed, ${queue.length} left`);
      } catch (e) {
        failed++;
  ...
  console.log(`\nDone: ${fetched} fetched, ${cached} already cached, ${failed} failed → ${DECKS}`);
