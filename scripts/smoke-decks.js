...
// Imports the renderer ES modules with browser-global stubs, then exercises
// the deck parser, validator, stats, and exporters.
// Run: node scripts/smoke-decks.js
'use strict';
const noop = () => {};
...
(async () => {
  const { parseDeckLine, parseDeckText, parseDeckList, deckToText, deckToCsv } = await import('../src/renderer-js/deckIO.js');
  const { createDeck, addCardToDeck, validateDeck, deckStats } = await import('../src/renderer-js/decks.js');
  const { collection } = await import('../src/renderer-js/state.js');

  // ── parseDeckLine / parseDeckText ─────────────────────────────────────────
  const r = {};
  r.plain   = parseDeckLine('1 Sol Ring');
  r.xSuffix = parseDeckLine('4x Lightning Bolt');
  r.setNum  = parseDeckLine('1 Sol Ring (C21) 263');
  r.foil    = parseDeckLine('3 Verdant Catacombs (MH2) 260 *F*');
  r.etched  = parseDeckLine('1 Sol Ring (SLD) 1117 *E*');
  r.archi   = parseDeckLine('1x Krenko, Mob Boss (ddt) 46 [Premium,Commander{top}]');
  r.bare    = parseDeckLine('Counterspell');
  r.text = parseDeckText([
    'About',
    'Name Goblin Storm',
    '',
    'Commander',
