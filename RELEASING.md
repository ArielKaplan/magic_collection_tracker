...
Installed copies of the app pick it up via Settings â†’ Updates.
...
and sets it as the GitHub release body â€” which is what the in-app **What's New**
...
1. After CI finishes, check
   <https://github.com/sarcasticsoftwarestudio/magic_collection_tracker/releases> â€” the new
   release should have `Secret Lair Tracker Setup X.Y.Z.exe`, `latest.yml`,
   and a `.blockmap`.
2. Open an older installed copy of the app â†’ Settings â†’ **Check for Updates**.
...
- `CSC_LINK` â€” base64 of the `.pfx` file (or an `https://` URL)
- `CSC_KEY_PASSWORD` â€” the cert password

electron-builder picks them up automatically; no workflow changes needed.

## Notes

- Don't edit `latest.yml` by hand â€” it's generated and signed against the
  installer's blockmap.
- The startup check is silent (no toast unless an update exists). Manual checks
  happen in Settings â†’ Updates. Both are disabled in dev mode (`npm run dev`).
